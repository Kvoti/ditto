define(["exports"], function (exports) {
    ...
    // Chat public API ----------------------------------
    function connect(server, _jid, password, _chatroom, _nick) {
        var log = arguments[5] === undefined ? false : arguments[5];
    ...
    function sendPrivateMessage(to, message) {
        ...
        addMessage(Strophe.getNodeFromJid(jid), to, new Date(), message);
    };
    ...
    function sendIsTyping(to) {
        if (!composedMessageChangeAt[to]) {
            connection.chatstates.sendComposing(getBareJID(to));
            composedMessageChangeAt[to] = new Date();
            setTimeout(function () {
                checkImStillTyping(to);
            }, stillTypingTimeout);
        }
    }
    ...
    function setAvatar(avatar) {
        ...
        // TODO handle error
        connection.vcard.set(function (r) {}, // TODO handle something here?
        vcard);
    ...
    function emitChange() {
        change_callbacks.forEach(function (callback) {
            return callback();
        });
    };
    ...
    function joinChatroom() {
        ...
        connection.muc.join(chatroom, nick, handleGroupMessage, handleGroupPresence);
    };
    ...
    var handlePresence = function handlePresence(pres) {
        var msg = $(pres);
    ...
    var handlePrivateMessage = function handlePrivateMessage(msg) {
        var msg = $(msg);
        ...
        if (composing.length) {
            ...
            if (body) {
                addMessage(from, to, when, body);
            }
        }
        return true;
    };

    var handleArchivedPrivateMessage = function handleArchivedPrivateMessage(msg) {
        var msg = $(msg);
        if (msg.find("[type=groupchat]").length === 0) {
            ...
            addMessage(from, to, when, body, true // isRead  TODO can't pass args by name?
            );
        }
        return true;
    };

    var handleGroupMessage = function handleGroupMessage(msg) {
        var msg = $(msg);
        var body = msg.find("body:first").text();
        ...
        if (from) {
            ...
            // itself, not sure why
            addGroupMessage(from, when, body);
        }
        return true;
    };

    var handleGroupPresence = function handleGroupPresence(pres) {
        var msg = $(pres);
    ...
    var addMessage = function addMessage(from, to, when, message) {
        var isRead = arguments[4] === undefined ? false : arguments[4];
    ...
    function handleRoster(roster, item) {
        var friends = [];
        roster.forEach(function (friend, i) {
    ...
    function checkImStillTyping(to) {
        ...
        if (lastChange) {
            ...
            if (now - lastChange > stillTypingTimeout) {
                delete composedMessageChangeAt[to];
                connection.chatstates.sendActive(getBareJID(to));
            } else {
